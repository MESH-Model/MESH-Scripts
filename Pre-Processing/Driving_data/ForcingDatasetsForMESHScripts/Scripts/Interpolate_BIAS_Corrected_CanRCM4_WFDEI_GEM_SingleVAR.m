...
% This script reads and interpolates netCDF data and writes seq files for a given basin
% Adapted to read CanRCM4-WFDEI-GEM data, knowing how the data is
% structured within folders and inside the nc files. This dataset has a
% regular LAT-LON grid and thus is interpolated using "interp2"
...
% inputs include MESH Drainage Database - line 36
% Path and naming pattern of ncfiles is to be supplied on line 100
% Path and naming pattern of seq files (to be created) is to be supplied on
% line 124. Folders have to be pre-created by the user including
% folders for segments as defined by the P vector (yyy1-yyy2)
...
VAR1 = {'hus','pr','ps','rlds','rsds','wind','ta'};
basin = 'KL'; % basin ID to be inserted in the output file names
...
str1 = char(strcat('E:\280_CanRCM4_Cor_WFDEI-GEM-CaPA1\', sc, '\'));
...
p1 = 1;          % Process from the first segment
base = P(p1)-1   % base for the var summary vector  
n = 1;           % Frame Counters - they are the same if the data contains Feb29
...
for p = p1: size(P,2)-1    
    ...
    for i = 1 : NT : t1
        ...
        % Read one day worth of data - the whole field
        Data(:,:,:) = ncread(fn, char(VAR(x)),[1 1 f], [size(lon,1) size(lat,1) NT]);
        if mod(f-1,365*NT)==0 
            g = 1;
        end
    ...
    % unstable
        ...
        for j = 1: NT
            ...
            Data1 = Data(:,:,j)';
            Data1=interp2(lon,lat,Data1,xmodel,ymodel,'linear');
            figure; imagesc(flipud(Data1))
