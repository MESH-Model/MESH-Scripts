...
catchmentShapeFolder    = read_from_control(controlFolder/controlFile,'river_basin_shp_path')
...
simulationFolder = read_from_control(controlFolder/controlFile,'simulations_path')
...
Path( rootPath / domainFolder / simulationFolder ).mkdir(parents=True, exist_ok=True)
...
if visualizationFolder == 'default':
    visualizationFolder = 'visualization'
#Try to make the visualization file folder; does nothing if the folder already exists
Path( rootPath / domainFolder / visualizationFolder ).mkdir(parents=True, exist_ok=True)
...
Path( rootPath / domainFolder / forcingFolder ).mkdir(parents=True, exist_ok=True)
...
networkTopologyFolder   = read_from_control(controlFolder/controlFile,'input_topo_path')
zonalStatisticsFolder   = read_from_control(controlFolder/controlFile,'zonalstats_path')

#Specify the default paths if required
if drainageDatabaseFolder   == 'default':
    drainageDatabaseFolder  = 'parameters/drainagedatabase'
if networkTopologyFolder    == 'default':
    networkTopologyFolder   = 'parameters/topology'
if zonalStatisticsFolder    == 'default':
    zonalStatisticsFolder   = 'parameters/zonalhist'



#Try to make the parameter file folders; does nothing if the folder already exists
Path( rootPath / domainFolder / drainageDatabaseFolder  ).mkdir(parents=True, exist_ok=True)
Path( rootPath / domainFolder / networkTopologyFolder   ).mkdir(parents=True, exist_ok=True)
Path( rootPath / domainFolder / zonalStatisticsFolder   ).mkdir(parents=True, exist_ok=True)
Path( rootPath / domainFolder / forcingFolder           ).mkdir(parents=True, exist_ok=True)
